...
class BaselineServer:
    ...
    def _setup_routes(self):
        ...
        def notify():
            ...
            with self.lock:
                ...
                current_rps = (
                    self.request_count / current_uptime if current_uptime > 0 else 0
                )
                avg_processing_time = (
                ...
                self.logger.info(
                    f"Mensagem recebida. Total: {self.request_count}, Avg Proc Time: {avg_processing_time:.6f}s, RPS: {current_rps:.2f}"
                )

            return (
                jsonify({"status": "success", "processing_time": processing_time}),
                200,
            )
    ...
    def run(self, host: str = None, port: int = None, debug: bool = False):
        ...
        self.logger.info("🔗 Endpoints disponíveis:")
        self.logger.info("   • POST /notify - Receber notificações")
        self.logger.info("   • GET /stats - Estatísticas do servidor")
        self.logger.info("   • GET /health - Health check")
        self.logger.info("   • POST /reset - Resetar contadores")
