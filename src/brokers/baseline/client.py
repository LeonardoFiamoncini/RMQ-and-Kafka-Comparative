    ...
    def send_messages(self, count: int, message_size: int, rps: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if rps and rps > 0:
                ...
                self.logger.info(f"Rate Limiting ativado: {rps} RPS (intervalo: {sleep_interval:.6f}s)")
            
            # Verificar se o servidor está rodando
            try:
                health_response = requests.get(f"{self.base_url}/health", timeout=5)
                if health_response.status_code != 200:
                    self.logger.error(f"Servidor não está saudável: {health_response.status_code}")
                    return False
            ...
            for i in range(count):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.info(f"Mensagem {msg_id} enviada com sucesso - Tempo de processamento: {response.json().get('processing_time'):.6f}s")
                    else:
                        failed_requests += 1
                        self.logger.error(f"Falha ao enviar mensagem {msg_id}: Status {response.status_code}, Resposta: {response.text}")
                except requests.exceptions.Timeout:
                    failed_requests += 1
                    self.logger.error(f"Timeout ao enviar mensagem {msg_id}")
                except requests.exceptions.ConnectionError as e:
                    failed_requests += 1
                    self.logger.error(f"Erro de conexão ao enviar mensagem {msg_id}: {e}")
                except Exception as e:
                    failed_requests += 1
                    self.logger.error(f"Erro inesperado ao enviar mensagem {msg_id}: {e}")
            ...
            self.logger.info(f"Envio finalizado: {successful_requests} sucessos, {failed_requests} falhas")
...
def main():
    ...
    parser.add_argument('count', type=int, nargs='?', default=1000, 
                       help='Número de mensagens')
    parser.add_argument('size', type=int, nargs='?', default=100, 
                       help='Tamanho das mensagens')
    parser.add_argument('rps', type=int, nargs='?', default=None, 
                       help='Rate limiting (RPS)')
