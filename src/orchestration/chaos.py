...
from ..core.config import CHAOS_CONFIG
...
class ChaosEngineer:
    ...
    def kill_container(self, container_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(
                    f"❌ Erro ao terminar contêiner {container_name}: {result.stderr}"
                )
                return False
        except Exception as e:
    ...
    def wait_for_container_recovery(
        self, container_name: str, max_wait: int = None
    ) -> float:
        ...
        while time.time() - start_wait < max_wait:
            try:
                ...
                if result.returncode == 0 and "Up" in result.stdout:
                    ...
                    self.logger.info(
                        f"✅ Contêiner {container_name} recuperado em {recovery_time:.2f}s"
                    )
        ...
        self.logger.warning(
            f"⚠️ Timeout aguardando recuperação do contêiner {container_name}"
        )
        return max_wait
    ...
    def run_chaos_experiment(
        ...
        self.logger.info(
            f"🔥 Iniciando experimento de tolerância a falhas para {tech.upper()}"
        )
        ...
        if tech == "kafka":
            ...
            self.logger.error(
                f"❌ Tecnologia {tech} não suportada para experimento de chaos"
            )
            return {}
        ...
        def producer_worker():
            try:
                ...
                if tech == "kafka":
                    ...
                    producer_results["success"] = producer.send_messages(
                        count, size, rps
                    )
                elif tech == "rabbitmq":
                    ...
                    producer_results["errors"].append(
                        f"Tecnologia {tech} não suportada"
                    )
        ...
        recovery_time = (
            recovery_end - recovery_start if recovery_end and recovery_start else 0
        )
        ...
        self.logger.info(f"   • Tempo de recuperação: {recovery_time:.2f}s")
        self.logger.info(
            f"   • Tempo total do experimento: {total_experiment_time:.2f}s"
        )
