        ...
        try:
            ...
            for i in range(count):
                ...
                try:
                    ...
                    self.logger.info(f"Mensagem {msg_id} enviada e confirmada com {message_size} bytes")
                        
                except pika.exceptions.UnroutableError:
                    failed_deliveries += 1
                    self.logger.error(f"Mensagem {msg_id} não pôde ser roteada (UnroutableError)")
                except pika.exceptions.NackError:
                    failed_deliveries += 1
                    self.logger.error(f"Mensagem {msg_id} foi rejeitada pelo broker (NackError)")
            ...
            self.logger.info(f"Entrega finalizada: {successful_deliveries} sucessos, {failed_deliveries} falhas")

            # Salvar métricas
            additional_metrics = {
                'successful_deliveries': successful_deliveries,
                'failed_deliveries': failed_deliveries,
                'delivery_success_rate': (successful_deliveries / count * 100) if count > 0 else 0
            }
